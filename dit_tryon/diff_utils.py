...
class Diffusion(object):
    ...
    @torch.no_grad()
    def edm_sampler_with_conditions(self, x, t_steps, i, model, 
                                   cloth_agnostic, garment, garment_pose, target_pose,
                                   s_churn=0., s_min=0., s_max=float('inf'), s_noise=1.):
        """
        Modified sampler that passes conditions to the model
        """
        n = len(t_steps)
        gamma = self.get_gamma(t_steps[i], s_churn, s_min, s_max, s_noise, n)
        eps = torch.randn_like(x) * s_noise
        ...
        if gamma > 0: 
            ...
            x_hat = x
        d = self.get_d_with_conditions(model, x_hat, t_hat, cloth_agnostic, garment, garment_pose, target_pose)
        d_cur = (x_hat - d) / t_hat
        x_next = x_hat + (t_steps[i + 1] - t_hat) * d_cur
        if t_steps[i + 1] != 0: 
            d = self.get_d_with_conditions(model, x_next, t_steps[i + 1], cloth_agnostic, garment, garment_pose, target_pose)
            d_prime = (x_next - d) / t_steps[i + 1]
            d_prime = (d_cur + d_prime) / 2
            x_next = x_hat + (t_steps[i + 1] - t_hat) * d_prime
        return x_next
    ...
    def get_d_with_conditions(self, model, x, sig, cloth_agnostic, garment, garment_pose, target_pose):
        """
        Modified get_d that passes conditions to the model
        """
        c_skip, c_out, c_in = get_scalings(sig, self.sigma_data)
        sig = sig.view(-1)
        return model(x * c_in, sig, cloth_agnostic, garment, garment_pose, target_pose) * c_out + x * c_skip
