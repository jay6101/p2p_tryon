...
class TryonDataset(Dataset):
    ...
    def _load_metadata(self):
        ...
        for sample_id in sample_dirs:
            ...
            metadata[sample_id] = {
                ...
                'garment_path': os.path.join(sample_dir, '1.jpg'),
                'garment_mask_path': os.path.join(sample_dir, '1/alpha/1.png'),
                'garment_pose': os.path.join(sample_dir, '1_pose.jpg'),
    ...
    def __getitem__(self, idx):
        # Get target sample ID
        target_sample_id = list(self.metadata.keys())[idx]
        target_sample_data = self.metadata[target_sample_id]
        
        # Get a different sample ID for garment (ensure it's different from target)
        available_ids = list(self.metadata.keys())
        if len(available_ids) > 1:
            garment_sample_id = random.choice([sid for sid in available_ids if sid != target_sample_id])
        else:
            garment_sample_id = target_sample_id  # Fallback if only one sample
        garment_sample_data = self.metadata[garment_sample_id]
        
        # Load target images from target sample
        target_img = np.array(PIL.Image.open(target_sample_data['target_path']))/255.0
        
        # Load garment images from garment sample
        garment_img = np.array(PIL.Image.open(garment_sample_data['garment_path']))/255.0
        original_garment_person = np.array(PIL.Image.open(garment_sample_data['target_path']))/255.0
        
        # Load cloth agnostic mask from target sample
        try:
            cloth_agnostic_mask = np.array(PIL.Image.open(target_sample_data['cloth_agnostic_mask_path']))/255.0
        except:
            try:
                cloth_agnostic_mask = np.array(PIL.Image.open(target_sample_data['cloth_agnostic_mask_path'].replace('1.png', '3.png')))/255.0
            except:
                try:
                    cloth_agnostic_mask = np.array(PIL.Image.open(target_sample_data['cloth_agnostic_mask_path'].replace('1.png', '2.png')))/255.0
                except:
        ...
        # Load garment mask from garment sample
        try:
            garment_mask = np.array(PIL.Image.open(garment_sample_data['garment_mask_path']))/255.0
        except:
            try:
                garment_mask = np.array(PIL.Image.open(garment_sample_data['garment_mask_path'].replace('1.png', '3.png')))/255.0
            except:
                try:
                    garment_mask = np.array(PIL.Image.open(garment_sample_data['garment_mask_path'].replace('1.png', '2.png')))/255.0
                except:
                    garment_mask = np.zeros((1024,768))
        garment_img = (garment_mask[:,:,None])*(garment_img)
        
        # Load pose data from respective samples
        target_pose = np.array(PIL.Image.open(target_sample_data['target_pose']))/255.0
        garment_pose = np.array(PIL.Image.open(garment_sample_data['garment_pose']))/255.0
        
        # Apply the same augmentation to target image, cloth agnostic, and target pose
        target_img, cloth_agnostic_img, target_pose, garment_img, garment_pose, original_garment_person = self.apply_augmentation([
            target_img, cloth_agnostic_img, target_pose, garment_img, garment_pose, original_garment_person
        ])
        ...
        original_garment_person_tensor = self.base_transform(original_garment_person.astype(np.float32))
        ...
        return {
            ...
            'original_garment_person': original_garment_person_tensor,
            'target_sample_id': target_sample_id,
            'garment_sample_id': garment_sample_id
        }
