...
def train(model_dir, data_dir, fid_real_dir, 
    ...
    steps = conf.steps
    sampling_steps = conf.sampling_steps
    ...
    sched = torch.optim.lr_scheduler.LinearLR(
                optimizer,
                start_factor = 1e-3,   # grows from 3e-7 → 3e-4
    ...
    # plot shape
    sz = (plot_shape[0], 3, img_size[0], img_size[1])
    ...
    for idx in pbar:
        ...
        garment_rgb   = cloth_agnostic_input[:, :3]                        # (B,3,H,W)
        garment_mask  = (garment_rgb.abs().sum(dim=1, keepdim=True)==-3).float()
        pix_l2 = (outputs - target).pow(2)
        pix_l1 = (outputs - target).abs()
        lambda_garment = 3.0
        weighted_pix_l2 = pix_l2 * (1.0 + lambda_garment * garment_mask)
        weighted_pix_l1 = pix_l1 * (1.0 + lambda_garment * garment_mask)
        rec_loss = (weighted_pix_l2.mean() + 0.1 * weighted_pix_l1.mean())
        
        weight = (t**2 + diffusion.sigma_data**2)/(t**2 * diffusion.sigma_data**2)
        loss = weight*rec_loss
        loss = loss.mean()
        # weight = (t**2 + diffusion.sigma_data**2)/(t**2 * diffusion.sigma_data**2)
        ...
        if i % iter_interval == 0:
            ...
            gen_batch = sample_with_conditions_cfg(
                diffusion, ema, batch_data, sz, guidance_scale=3, steps=sampling_steps, seed=seed
            )
...
def main():
    ...
    parser.add_argument('--iter_interval', type=int, default=5000)
