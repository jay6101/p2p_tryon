'''DiT Transformer model for PyTorch with improved positional embedding and conv decoder head.  
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, t_emb, cloth_tokens, garment_tokens):
        ...
        h = self.ln1(x)
        x = x + self.attn(h)
        # Cross-attn conditioning on garment
        ...
        # MLP
        shift_m = self.beta_mlp(t_emb)
        scale_m = self.gamma_mlp(t_emb)
...
class DiT(nn.Module):
    def __init__(self, img_size, dim=64, patch_size=4,
        ...
        self.n_patches = H * W
        # improved 2D positional embedding
        ...
        # pixelshuffle + conv decoder head
    ...
    def forward(self, x, t, cloth_agnostic, garment):
        # time embedding - reshape t to ensure correct dimensions
        # t = t.reshape(-1, 1)  # Ensure t is [B, 1]
        ...
        # final
        out = self.final(x_tok, t_emb)  # [B, N, p*p*ch]
